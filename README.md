<!--
    Dear user using my README as a base
    to create your own, I’m happy to authorize its use 
    and I’m glad you liked it! I just kindly ask for one thing:

    Please, leave a star on the original README it would truly make my day :)
    Original by: https://github.com/Glauedson
    Adapted for: Mariana Moreira
-->

<!-- Banner -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:050C12,50:111111,100:050C12&height=180&section=header&text=MARIANA%20MOREIRA&fontSize=42&fontColor=ffffff&fontAlignY=35&desc=Developer&descAlignY=55&descSize=18" width="100%"/>
</div>

<br/>

<!-- Profile badges -->
<div align="center">

  <a href="https://github.com/Maahh-Moreira">
    <img width="120px" 
        alt="Stars" 
        title="Stars Profile" 
        src="https://custom-icon-badges.demolab.com/badge/dynamic/json?logo=star&color=000000&labelColor=ffffff&logoColor=000000&label=Stars&style=for-the-badge&query=%24.stars&url=https://api.github-star-counter.workers.dev/user/MarianaMoreira" />
  </a>

  <a href="https://github.com/Maahh-Moreira">
    <img width="115px" 
        alt="Views" 
        title="Profile Views" 
        src="https://komarev.com/ghpvc/?username=MarianaMoreira&label=Views&color=000000&style=for-the-badge" />
  </a>

  <a href="https://github.com/Maahh-Moreira">
    <img width="140px" 
        alt="Followers" 
        title="Github Followers" 
        src="https://custom-icon-badges.demolab.com/github/followers/MarianaMoreira?color=000000&labelColor=ffffff&style=for-the-badge&logo=person-add&label=Followers&logoColor=000000" />
  </a>

</div>

<br/>

<!-- Who am I? -->
<img 
  src="https://media.tenor.com/Gh3LKX9HMFkAAAAj/hollow-knight-knight.gif" 
  width="320" 
  align="left" 
  style="filter: grayscale(100%)"
/>

<div align="center">

### Who Am I?

</div>

<div align="justify">

I am a **Systems Development Technician** currently pursuing a degree in `Information Systems`. I am passionate about **software development, problem-solving, and creating technology that turns ideas into practical solutions**.

I currently work at a **software development and technical support company**, where I work with **application development, system maintenance, troubleshooting, and technical support**. This experience has given me the opportunity to work with real-world systems while developing both my **technical skills and professional mindset**.

My main interests lie in `Software Development`, `Web Development`, and `Application Development`, especially `Front-End Development`, where **logic meets creativity and user experience**. I also enjoy working with databases, exploring new technologies, debugging systems, and understanding how different pieces of software come together.

Throughout my academic and professional journey, I have worked on **personal, academic, and real-world projects**, constantly improving my ability to **learn independently, solve problems, adapt to new technologies, and build functional solutions**.

Currently, my goal is simple: **keep learning, keep building, and become a better developer every day.**

</div>

<br/><br/>

<!-- Social -->
<div align="center">

  **You can click here**

  <a href="https://github.com/MarianaMoreira">
    <img 
      width="110px"
      src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white" 
      alt="GitHub"
      title="My GitHub"
    />
  </a>

  <a href="https://www.linkedin.com/in/mariana-aparecida-peguim-moreira-084060394">
    <img 
      width="110px"
      src="https://img.shields.io/badge/LinkedIn-000000?style=for-the-badge&logo=linkedin&logoColor=white" 
      alt="LinkedIn"
      title="My LinkedIn"
    />
  </a>

</div>

<br/>

<!-- Curious -->
<img src="bananinha.png" width="300" align="right"/>

> [!NOTE]
>
> **Behind the Code**
>
> I’m constantly learning, creating, and turning ideas into something real.
>
> Every project is a chance to **learn something new** and **build something better**.

<br/>

<br clear="both"/>

<!-- My contributions title -->
<div align="center">
  <h3>My Contributions</h3>
</div>

<br clear="both"/>

<!-- Skills + Stats side by side -->
<table align="center">
<tr>

<!-- TECH STACK -->
<td width="50%" valign="top" align="center">

<b>Tech Stack</b>

<br/><br/>

<table align="center">
<tr>

<!-- Languages -->
<td align="center" valign="middle">

<img src="https://skillicons.dev/icons?i=html,css,js,python,php" /><br/>
<img src="https://skillicons.dev/icons?i=cs,git,github,arduino,mongodb" /><br/>
<img src="https://skillicons.dev/icons?i=mysql,postgres,vscode,discord,androidstudio" />

</td>

</tr>
</table>

</td>

<!-- LATEST ACTIVITY -->
<td width="50%" valign="top" align="center">

<b>Latest Activity</b>

<br/><br/>

[![GitHub Streak](https://streak-stats.demolab.com?user=MarianaMoreira&theme=dark&background=0D0D0D&ring=ffffff&fire=ffffff&currStreakNum=FFFFFF&sideNums=FFFFFF&currStreakLabel=ffffff&sideLabels=aaaaaa&dates=aaaaaa&card_width=340)](https://git.io/streak-stats)

<br/>

</td>

</tr>
</table>

<br/>

<!-- Footer -->
<div align="center">
  <img 
    src="https://capsule-render.vercel.app/api?type=waving&color=050C12&height=100&section=footer"
    width="100%"
  />
</div>

<div align="center">
  <sub>Mariana Moreira · Developer</sub>
</div>
